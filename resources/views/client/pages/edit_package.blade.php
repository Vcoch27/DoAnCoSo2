...
<div class="container my-4" style=" position: relative;">
    <!-- 
    <pre>{{ json_encode($questionPackage, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
    <pre>{{ json_encode($tags, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
    ...
    <form action="{{ route('packages.update', $questionPackage->id) }}" method="POST">
        @csrf
        @method('PUT')
        <!-- Header and Buttons -->
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2>Edit Packages Questions For You</h2>

            <div>

                <button type="submit" class="btn btn-secondary me-2">Confirm</button>
                <button class="btn btn-primary">Confirm and practice</button>
            </div>
        ...
        <div class="mb-3">
            ...
            <div class="d-flex align-items-center mb-2">
                <label class="switch me-3">
                    <input type="checkbox" name="register_public" id="register_public" {{ $questionPackage->public ? 'checked' : '' }}>
            ...
            <span>
                <u>Note</u>: Even if you edit it, the request will be sent to <strong>public</strong> again, and your <strong>package</strong> will be temporarily set to <strong>private</strong>.
            </span>
        ...
        <div class="mb-3">
            ...
            <input type="text" name="title" class="form-control" placeholder="Enter a title, like 'Biology - Chapter 22: Evolution'" value="{{ $questionPackage['title']}}">
        </div>
        <div class="mb-3">
            <label for="title">
                <h6>Description:</h6>
            </label>
            <textarea name="description" class="form-control" placeholder="Add a description...">{{ $questionPackage['description']}}</textarea>
        </div>
        ...
        <div class="d-flex mb-3">
            ...
            <div class="menu">
                <div class="tags-input-container">
                    @foreach($questionPackage['tags'] as $tag)
            ...
            <div class="card__tags">
                <ul class="tag">
                    @foreach ($questionPackage['tags'] as $tag)
                    <li class="tag__name" data-id={{$tag->id}}>{{$tag->name}} <sup>x</sup></li>
                    @endforeach
                </ul>
            </div>
        </div>
        ...
        const answersHTML = question.answers.map((answer, i) => `
                    <div class="form-check d-flex justify-content-between align-items-center option-item">
                        <label class="container-checkbox">
                            ...
                            ${answer.is_correct === 1? "checked" : ""}${answer.is_correct === "on"? "checked" : ""}>
                            <div class="checkmark"></div>
                        </label>
                        <div class="input-wrapper">
                            <input type="text" class="option-input" placeholder="Enter option" 
                                ...
                                <input type="hidden" 
                                value="${answer.id || ""}" 
                                name="questions[${questionIndex}][answers][${i}][id]">
...
<!-- Nội dung package mặc định -->
<script>
    // tạo giá trị mặc định của tags
    function updateHiddenInput() {
        const tagsInputContainer = document.querySelector(".tags-input-container");
        // Lấy tất cả các input ẩn đã tạo từ các tag
        const hiddenInputs = Array.from(tagsInputContainer.querySelectorAll("input[name='tags[]']"));
    ...
    // Gắn sự kiện cho từng tag mặc định
    function attachRemoveEvent(tag) {
        const removeButton = tag.querySelector("sup");
        if (removeButton) {
            removeButton.addEventListener("click", function() {
                tag.remove(); // Xóa tag khỏi danh sách
                updateHiddenInput(); // Cập nhật input ẩn
            });
        }
    }
    // Xử lý các tag mặc định
    const defaultTags = document.querySelectorAll(".tag__name"); // Chọn tất cả <li> trong .tag
    defaultTags.forEach(tag => {
        attachRemoveEvent(tag); // Gắn sự kiện xóa cho từng tag
    });
</script>
<script>
    // Các phần tử
